...
on:
  ...
  roles: all
  # Fork-only E2E entry point: analyze a specific Azure DevOps source PR/build
  # while binding all safe outputs to a mirrored PR in this fork.
  workflow_dispatch:
    inputs:
      ado-build-id:
        ...
        type: string
      ado-pr-number:
        description: "Source dotnet/msbuild PR number recorded by the Azure build."
        required: true
        type: string
      pr-number:
    ...
    steps:
      - name: Download binlogs from the failed Azure Pipelines build
        ...
        run: |
          ...
          case "${BASE_REF}" in
            main|vs*|e2e-build-failure-base) echo "PR #${PR_NUMBER} base '${BASE_REF}' is in scope." ;;
          ...
          #        belong to the source PR (sourceBranch == refs/pull/<PR>/merge).
          ...
          # the later point-in-time, agent, and safe-output checks.
...
safe-outputs:
  ...
  # The fetch job separately verifies that the ADO build belongs to the
  # explicit source PR before the agent can run.
