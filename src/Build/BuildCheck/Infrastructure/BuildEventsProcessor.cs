...
internal class BuildEventsProcessor(BuildCheckCentralContext buildCheckCentralContext)
{
    ...
    private Dictionary<string, string> _evaluatedEnvironmentVariables = new Dictionary<string, string>();

    /// <summary>
    /// The method collects events associated with the used environment variables in projects.
    /// </summary>
    internal void ProcessEnvironmentVariableReadEventArgs(string environmentVariableName, string value)
    {
        if (!_evaluatedEnvironmentVariables.ContainsKey(environmentVariableName))
        {
            _evaluatedEnvironmentVariables.Add(environmentVariableName, value);
        }
    }
